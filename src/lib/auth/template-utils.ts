 ...
 */
export function hasPermission(locals: App.Locals, permission: Permission): boolean {
 ...
 */
export function hasAnyPermission(locals: App.Locals, permissions: Permission[]): boolean {
    if (!locals.user?.permissions) {
        return false
    }

    return permissions.some(permission => locals.user!.permissions.includes(permission))
}

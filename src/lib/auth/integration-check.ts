...
import type { Role, Permission, UserContext, MenuItem, RouteConfig } from '@/types/auth'
import {
    DEFAULT_ROLE,
    ROLE_PERMISSIONS,
    PROTECTED_ROUTES
...
function verifyTemplateUtilities(): boolean {
    const mockLocals: App.Locals = {
        user: {
            ...
            email: 'test@example.com',
            role: 'admin',
            permissions: ['write_content', 'edit_content', 'manage_user']
        }
    }
...
// Export for testing
export { verifyTypes, verifyConfiguration, verifyUtilities, verifyTemplateUtilities }